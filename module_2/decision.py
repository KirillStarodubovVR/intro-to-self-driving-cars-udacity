...
"""Complete the function below so that it returns the string L when option 1
is a shorter distance and returns the string R when option 2 is shorter.
If the two options are the same it can return either L or R."""

def make_decision(L1, L2, L3, L4):
    # try to solve this problem directly
    if (L1+L2)<=(L3+L4):
        return "R"
    else:
        return "L"
